...
const Navbar = () => {
  ...
  const controlNavbar = () => {
    if (typeof window !== "undefined") {
      if (window.scrollY > lastScrollY && window.scrollY > 0) {
        // If scroll down and not at the top, hide the navbar
        setShowNavbar(false);
      } else {
        // If scroll up or at the top, show the navbar
        setShowNavbar(true);
      }
  ...
  return (
    <div
      className={`fixed z-50 w-full px-6 py-6 flex justify-between items-center transition-transform duration-300 ${
        showNavbar
          ? "translate-y-0 bg-opacity-30 backdrop-blur-md rounded-lg"
          : "-translate-y-full"
      }`}
    >
      <div className="logo text-[#212121]">
      ...
      <div className="links flex gap-6">
        {["Services", "Our Work", "About Us", "Insight", "Contact us"].map(
          (item, index) => (
            <a
              key={index}
              className={`text-lg font-regular font-sans text-[#212121] capitalize ${
                index === 4 && "ml-32"
              } custom-underline`}
            >
              {item}
